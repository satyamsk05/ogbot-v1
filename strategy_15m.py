...
class Strategy15M:
    ...
    def process(self, client, live_data, current_balance, bot_mode):
        ...
        # 15m: two RED -> bet GREEN (UP)
        # 15m: two GREEN -> bet RED (DOWN)
        last2 = [c['color'] for c in live_data['candles'][-2:]]
        signal = ""
        if all(x == "RED" for x in last2):
            signal = "UP"
        elif all(x == "GREEN" for x in last2):
            signal = "DOWN"
        ...
        if signal and bot_mode == "AUTO":
            ...
            if not is_valid:
                ...
                if success:
                    ...
                    # Store expiry timestamp from slug logic
                    try:
                        # Attempt to cast directly to int, as timestamps are typically integers
                        self.active_bet_expiry = int(self.active_bet_slug.split('-')[-1])
                    except ValueError:
                        # If direct int conversion fails (e.g., it's a float string), try float then int
                        try:
